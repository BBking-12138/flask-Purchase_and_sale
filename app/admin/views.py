...
def login():
    ...
    if form.validate_on_submit():
        data = form.data
        # print("data:", data)
        admin = User.query.filter_by(count=data['count']).first()
        ...
        session["admin"] = data['count']
        # if power_name is not None and len(power_name) > 0:
        #     session['power'] = power_name[0]
        # else:
        #     # 处理 power_name 为 None 或者为空的情况
        #     session['power'] = 'default_value'
...
def register():
    ...
    if form.validate_on_submit():
        data = form.data
        names = User.query.filter_by(count=data['count']).count()
        if names == 1:
            flash('注册失败')
...
# 添加招标信息
@admin.route("/increasebidsuccessfulorder/",methods=["GET","POST"])
@admin_login_req
def increasebidsuccessfulorder():
    form=IncreaseBidSuccessfulOrder()
    if form.validate_on_submit():
        data = form.data
        project = Project(
    ...
    return render_template("admin/increasebidsuccessfulorder.html",form=form)
...
# 修改招标信息
@admin.route("/tenderrevise/",methods=["GET","POST"])
@admin_login_req
def tenderrevise():
    form = TenderRevise()
    if not form.id.data:
...
def persionDetail():
    form=alertpasswd()
    usermessage=User.query.filter(User.count==session["admin"]).first()
    admin = User.query.filter_by(count=usermessage.count).first()
    if form.validate_on_submit():
...
def wjmm():
    form = wjpasswd()
    if form.validate_on_submit():
        usermessage = User.query.filter(User.count == form.data["count"]).first()
        if usermessage is None:
            flash("账号错误！")
            return render_template("admin/wjmm.html", form=form)
        admin = User.query.filter_by(count=usermessage.count).first()

        if admin.email != form.data['email']:
            flash('请输入正确的邮箱地址，或联系管理员修改')
            return render_template("admin/wjmm.html", form=form)

        admin.password = generate_password_hash((form.data['password']))
        mails=[]
